...
def test_unit_task_manager_can_mark_task_complete(manager):
    ...
    required_task = manager.get_task(task_id)
    required_task_status = required_task["completed"]
    assert required_task_status == True
...
def test_integration_api_creates_task(test_app, manager):
    ...
    required_task = manager.get_task_by_title("Integration Test Task")
    assert required_task["title"] == api_payload["title"] # Placeholder
...
def test_e2e_user_can_create_task_via_ui(selenium_driver):
    ...
    add_button.click()
    aux = selenium_driver.find_element(By.ID, "task-list")
    ...
    assert aux.text == "E2E Test Task Title" # Placeholder
